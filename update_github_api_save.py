...
<head>
    ...
    <style>
        ...
        * {{ box-sizing: border-box; margin: 0; padding: 0; }}
        ...
        /* Modal Overlay for GitHub Token Settings */
        .modal-overlay {{
            position: fixed;
...
<body>
    ...
    <main class="container">
        ...
        <div class="view-section active" id="viewQuestions">
            ...
            <div class="toolbar">
                ...
                <div class="tool-group">
                    <button class="btn" onclick="saveData()"><i class="fa-solid fa-cloud-arrow-up"></i> Save to GitHub JSON</button>
                    <button class="btn btn-github" onclick="openGitHubModal()"><i class="fa-brands fa-github"></i> GitHub PAT Settings</button>
    ...
    <div class="modal-overlay" id="githubModal">
        <div class="modal-card">
            <div class="modal-title"><i class="fa-brands fa-github"></i> GitHub Sync Settings</div>
            <p style="font-size: 0.88rem; color: var(--text-secondary); margin-bottom: 15px;">
                To allow direct 1-click saving of answers to the <strong>KarmaYogaQuestionsDiscusssions</strong> GitHub repository, enter your GitHub Personal Access Token (PAT) below. It will be stored securely in your browser's LocalStorage.
            </p>
            <div class="form-group">
                <label>GitHub Personal Access Token (PAT):</label>
                <input type="password" id="ghTokenInput" class="search-input" style="padding-left: 14px;" placeholder="ghp_xxxxxxxxxxxxxxxxxxxx">
            </div>
            <div style="display: flex; justify-content: flex-end; gap: 10px; margin-top: 20px;">
                <button class="btn btn-secondary" onclick="closeGitHubModal()">Cancel</button>
                <button class="btn" onclick="saveGitHubToken()">Save Token</button>
            </div>
        </div>
    </div>

    <div class="notification" id="notification">
        <i class="fa-solid fa-circle-check"></i> JSON Saved!
    </div>
    ...
    <script>
        ...
        const GH_REPO_OWNER = 'karmayogabg';
        const GH_REPO_NAME = 'KarmaYogaQuestionsDiscusssions';
        const GH_FILE_PATH = 'questions_data.json';
                        ...
                        <div class="question-card" id="card-${{q.id}}">
                            ...
                            <div class="card-body">
                                ...
                                <div class="answer-editor-box">
                                    ...
                                    <button class="save-card-btn" onclick="saveData()"><i class="fa-solid fa-cloud-arrow-up"></i> Save to GitHub JSON</button>
                                </div>
                            </div>
                        </div>
                    `;
        ...
        async function saveData() {{
            ...
            // 1. Try Local Backend Server (/api/save)
            try {{
                ...
                if (res.ok) {{
                    ...
                    showNotification('✓ ' + (result.message || 'Saved to local questions_data.json!'));
                    return;
                }}
            }} catch (e) {{}}

            // 2. Try GitHub REST API Direct Save
            const token = localStorage.getItem('gh_pat_token');
            if (token) {{
                try {{
                    // Get current file sha
                    const getUrl = `https://api.github.com/repos/${{GH_REPO_OWNER}}/${{GH_REPO_NAME}}/contents/${{GH_FILE_PATH}}`;
                    ...
                    const putBody = {{
                        message: "Update answers and discussion statuses in questions_data.json via web manager",
                    ...
                    if (putRes.ok) {{
                        showNotification('✓ Updated & Committed questions_data.json directly on GitHub!');
                        return;
                    }} else {{
                        const err = await putRes.json();
                        console.error('GitHub API error:', err);
                        showNotification('GitHub API Save Error: ' + (err.message || 'Check PAT permissions'));
                        openGitHubModal();
                        return;
                    }}
                }} catch (e) {{
                    console.error('GitHub REST API failed:', e);
                }}
            }} else {{
                openGitHubModal();
            }}
        ...
        function saveGitHubToken() {{
            ...
            if (token) {{
                ...
                showNotification('GitHub Personal Access Token saved!');
                closeGitHubModal();
                saveData();
            }}
        }}
...
print('Updated index.html, Karmayoga-Questions-Discussion.html, and artifact with GitHub REST API Direct Save!')
